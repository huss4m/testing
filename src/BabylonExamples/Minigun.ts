...
export class Minigun extends Weapon {
    ...
    async CreateG(): Promise<void> {
        ...
        this.reloadSound = new Sound(
            "reloadSound",
            "./audio/minigunreload.mp3", // Chemin vers le fichier audio
            this.scene,
            null,
            {
                spatialSound: true,
    ...
    shootAnimation(gun: Weapon) {
        ...
        gun.animationGroups.forEach( (animationGroup) => {
            animationGroup.stop();
            animationGroup.start(false, 1, 0, 21);
...
createMuzzleFlashParticleSystem(): void {
    ...
    this.muzzleFlashParticleSystem.minEmitBox = new Vector3(-0.2, -0.25, 5); // Adjust position
    this.muzzleFlashParticleSystem.maxEmitBox = new Vector3(-0.2, -0.25, 5); // Adjust position
